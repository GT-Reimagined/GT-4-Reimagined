...
public class BlockColoredWall extends BlockFakeTile implements IColorHandler {
    ...
    public BlockColoredWall(String domain, Material material){
        this(domain, material, Properties.of(net.minecraft.world.level.material.Material.METAL).strength(1.0f, 10.0f).sound(SoundType.METAL).requiresCorrectToolForDrops());
    }
    ...
    public Texture[] getTextures() {
        Texture side = material == Wood ? new Texture(GTCore.ID, "block/casing/wall/wood") : new Texture(GT4RRef.ID, "block/casing/wall/metal");
        Texture overlay = new Texture(GTCore.ID, "block/machine/empty");
        Texture sideOverlay = material == Wood ? new Texture(GTCore.ID, "block/casing/wall/wood_overlay_side") : overlay;
