...
public class ForgeHammerLoader {
    public static void init(){
        GTMaterialTypes.CRUSHED_ORE.all().forEach(m -> {
            ...
            int multiplier = 1;
            RecipeIngredient ore = RecipeIngredient.of(TagUtils.getForgelikeItemTag("sandless_ores/" + m.getId()),1), crushed = GTMaterialTypes.CRUSHED_ORE.getIngredient(m, 1);
            ...
            FORGE_HAMMER.RB().ii(crushed).io(IMPURE_DUST.get(MACERATE_INTO.getMapping(m), 1)).add(m.getId() + "_crushed_ore",16, 10);
            FORGE_HAMMER.RB().ii(RecipeIngredient.of(PURIFIED_ORE.get(m,1))).io(PURE_DUST.get(MACERATE_INTO.getMapping(m), 1)).add(m.getId() + "_purified_ore",16, 10);
            if (m.has(REFINED_ORE)) {
                FORGE_HAMMER.RB().ii(RecipeIngredient.of(REFINED_ORE.get(m,1))).io(DUST.get(MACERATE_INTO.getMapping(m), 1)).add(m.getId() + "_centrifuged_ore",16, 10);
            }
            if (m.has(RAW_ORE)){
        ...
        LONG_ROD.all().stream().filter(m -> !m.has(MaterialTags.NOSMASH)).forEach(rod -> {
            FORGE_HAMMER.RB().ii(ROD.getMaterialIngredient(rod, 2)).io(LONG_ROD.get(rod, 1)).add("rod_long_" + rod.getId(), rod.getMass(), 16);
        });
        ...
        FORGE_HAMMER.RB().ii(RecipeIngredient.of(Items.BRICK, 1)).io(SMALL_DUST.get(Brick, 1)).add("brick_dust_small",10, 16);
