...
public class FilterButtonArrayWidget extends Widget {
    ...
    protected FilterButtonArrayWidget(@Nonnull GuiInstance instance, @Nullable IGuiElement parent) {
        ...
        this.redstoneControlWidget = (ChangingButtonWidget) ChangingButtonWidget.build(instance.handler.getGuiTexture(), redstoneControlLoc, null, GuiEvent.EXTRA_BUTTON,1).setSize(19, 1, 16, 16).buildAndAdd(instance, this);
        this.redstoneControlWidget.setStateHandler(w -> outputRedstone).setDepth(depth() + 1);
        this.invertRedstoneWidget = (ChangingButtonWidget) ChangingButtonWidget.build(instance.handler.getGuiTexture(), invertRedstoneLoc, null, GuiEvent.EXTRA_BUTTON,2).setSize(37, 1, 16, 16).buildAndAdd(instance, this);
        this.invertRedstoneWidget.setStateHandler(w -> invertRedstone).setDepth(depth() + 1);
        this.blacklistWidget = (ChangingButtonWidget) ChangingButtonWidget.build(instance.handler.getGuiTexture(), blacklistLoc, null, GuiEvent.EXTRA_BUTTON,3).setSize(55, 1, 16, 16).buildAndAdd(instance, this);
        this.blacklistWidget.setStateHandler(w -> blacklist).setDepth(depth() + 1);
        this.nbtWidget = (ChangingButtonWidget) ChangingButtonWidget.build(instance.handler.getGuiTexture(), nbtLoc, null, GuiEvent.EXTRA_BUTTON,4).setSize(73, 1, 16, 16).buildAndAdd(instance, this);
