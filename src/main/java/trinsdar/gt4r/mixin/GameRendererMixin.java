...
public abstract class GameRendererMixin {
    ...
    private double getExtendedAttackReach(double value) {
        ...
        return mc.player.getAttributeValue(GT4RData.ATTACK_REACH.get()) * 2.0D;
    }
    ...
    private double getAttackReach(double value) {
        if (mc.player == null) return value;
        return mc.player.getAttributeValue(GT4RData.ATTACK_REACH.get());
    }
    ...
    private double getAttackReachSquared(double value) {
        if (mc.player == null) return value;
        double attackReachValue = mc.player.getAttributeValue(GT4RData.ATTACK_REACH.get());
