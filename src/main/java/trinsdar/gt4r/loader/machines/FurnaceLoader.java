...
public class FurnaceLoader {
    public static void loadRecipes(Consumer<IFinishedRecipe> output, AntimatterRecipeProvider provider){
        ...
        DUST.all().forEach(m -> {
            ...
            CookingRecipeBuilder.blasting(DUST.getMaterialIngredient(m, 1).get(), INGOT.get(m.getDirectSmeltInto()), 0.5F, 100).unlockedBy("has_" + m.getId() + "_dust", provider.hasSafeItem(DUST.getMaterialTag(m))).save(output, m.getId() + "_dust_to_ingot_bl");
            CookingRecipeBuilder.smelting(DUST.getMaterialIngredient(m, 1).get(), INGOT.get(m.getDirectSmeltInto()), 0.5F, 200).unlockedBy("has_" + m.getId() + "_dust", provider.hasSafeItem(DUST.getMaterialTag(m))).save(output, m.getId() + "_dust_to_ingot");
        });
