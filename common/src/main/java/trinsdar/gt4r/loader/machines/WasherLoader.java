...
public class WasherLoader {
    public static void init() {
        CRUSHED.all().forEach(m -> {
            ...
            ORE_WASHING.RB().fi(AntimatterMaterials.Water.getLiquid(1000)).ii(of(CRUSHED.get(m,1))).io(CRUSHED_PURIFIED.get(m,1), DUST_TINY.get(aOreByProduct1,1), stoneDust).add(200, 24);
        });
    }
}
