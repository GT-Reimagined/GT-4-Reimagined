...
public class MaceratorLoader {
    public static void initManual(){
        ...
        MACERATING.RB().ii(RecipeIngredient.of(ItemTags.LOGS, 1)).io(DUST.get(AntimatterMaterials.Wood, 6)).add(400, 2);
        MACERATING.RB().ii(RecipeIngredient.of(Items.PRISMARINE, 1)).io(DUST.get(AntimatterMaterials.Prismarine, 1)).add(400, 2);
        MACERATING.RB().ii(RecipeIngredient.of(Items.DARK_PRISMARINE, 1)).io(DUST.get(AntimatterMaterials.DarkPrismarine, 1)).add(400, 2);
    ...
    public static void initAuto() {
        ...
        CRUSHED.all().forEach(m -> {
            if (!m.has(ORE) && m != AntimatterMaterials.NetheriteScrap) return;
            int multiplier = 1;
            ...
            if (m == AntimatterMaterials.NetheriteScrap){
                MACERATING.RB().ii(ore).io(Utils.ca((ORE_MULTI.getInt(m) * multiplier) * 2, crushedStack), DUST.get(oreByProduct1, 1), DUST.get(AntimatterMaterials.Netherrack, 1)).chances(1.0, 0.1 * multiplier * BY_PRODUCT_MULTI.getInt(m), 0.5).add(400, 2);
            }
        ...
        DUST.all().forEach(m -> {
            if (m.has(PLATE) && m != AntimatterMaterials.Wood){
        ...
        AntimatterAPI.all(StoneType.class, s -> {
            if (s.getMaterial() == Material.NULL || !s.getMaterial().has(DUST)) return;
        ...
        AntimatterAPI.all(FluidPipe.class).forEach(t -> {
            if (!t.getMaterial().has(DUST) || t.getMaterial() == AntimatterMaterials.Wood) return;
