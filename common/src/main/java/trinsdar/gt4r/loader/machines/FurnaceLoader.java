...
public class FurnaceLoader {
    public static void loadRecipes(Consumer<FinishedRecipe> output, AntimatterRecipeProvider provider){
        SimpleCookingRecipeBuilder.blasting(INGOT.getMaterialIngredient(AntimatterMaterials.Iron, 1), INGOT.get(WroughtIron), 0.5F, 100).unlockedBy("has_iron_ingot", provider.hasSafeItem(INGOT.getMaterialTag(AntimatterMaterials.Iron))).save(output, Ref.ID + ":iron_to_wrought_bl");
        SimpleCookingRecipeBuilder.smelting(INGOT.getMaterialIngredient(AntimatterMaterials.Iron, 1), INGOT.get(WroughtIron), 0.5F, 200).unlockedBy("has_iron_ingot", provider.hasSafeItem(INGOT.getMaterialTag(AntimatterMaterials.Iron))).save(output, Ref.ID + ":iron_to_wrought");
        ...
        AntimatterCookingRecipeBuilder.blastingRecipe(DUST.getMaterialIngredient(Tetrahedrite, 1), NUGGET.get(AntimatterMaterials.Copper, 6), 0.5F, 100).addCriterion("has_" + Tetrahedrite.getId() + "_dust", provider.hasSafeItem(DUST.getMaterialTag(Tetrahedrite))).build(output, Ref.ID + ":" + Tetrahedrite.getId() + "_dust_to_nuggets_bl");
        AntimatterCookingRecipeBuilder.smeltingRecipe(DUST.getMaterialIngredient(Tetrahedrite, 1), NUGGET.get(AntimatterMaterials.Copper, 6), 0.5F, 200).addCriterion("has_" + Tetrahedrite.getId() + "_dust", provider.hasSafeItem(DUST.getMaterialTag(Tetrahedrite))).build(output, Ref.ID + ":" + Tetrahedrite.getId() + "_dust_to_nuggets");
