...
public class CentrifugingLoader {
    public static void init() {
        ...
        CENTRIFUGING.RB().fi(Lava.getLiquid(100)).io(itemStacks).outputChances(0.2, 0.1, 0.025, 0.025, 0.01).add("lava",200, 16);
        CENTRIFUGING.RB().fi(FluidPlatformUtils.createFluidStack(GT4RData.PAHOEHOE_LAVA.getFluid(), 100 * TesseractGraphWrappers.dropletMultiplier)).io(itemStacks).outputChances(0.2, 0.1, 0.025, 0.025, 0.01).add("pahoehoe_lava", 200, 8);
        add(EnderEye, 10, 792);
        ...
        CENTRIFUGING.RB().ii(of(GRASS_BLOCK, 64)).io(new ItemStack(Items.SAND, 32), new ItemStack(CLAY_BALL, 2), new ItemStack(Plantball, 6)).add("grass",3125, 16);
        CENTRIFUGING.RB().ii(of(MYCELIUM, 64)).io(new ItemStack(Items.SAND, 32), new ItemStack(CLAY_BALL, 8), new ItemStack(BROWN_MUSHROOM, 16), new ItemStack(RED_MUSHROOM, 16)).add("mycelium",3906, 16);
        add(Lapis,20, 1792);
        CENTRIFUGING.RB().ii(of(StickyResin, 8)).io(DUST.get(Rubber, 21), DUST.get(Plastic, 7), new ItemStack(Plantball, 4)).add("sticky_resin",1250,10);
        CENTRIFUGING.RB().ii(of(DUST.getMaterialTag(Netherrack), 64)).io(new ItemStack(GOLD_NUGGET, 4), new ItemStack(REDSTONE, 4), new ItemStack(GUNPOWDER, 8), DUST.get(Coal, 4), DUST.get(Sulfur, 4), DUST.get(Phosphor, 2)).add("netherrack",3125,16);
        add(Granite, 16, 1500);
        ...
        CENTRIFUGING.RB().ii(of(Items.SOUL_SAND, 16)).io(new ItemStack(Items.SAND, 1), DUST.get(Saltpeter,4), DUST.get(Coal, 1)).fo(Oil.getLiquid(1000)).add("soul_sand",750, 16);
        CENTRIFUGING.RB().ii(of(Items.SOUL_SOIL, 16)).io(new ItemStack(DIRT, 1), DUST.get(Saltpeter,4), DUST.get(Coal, 1)).fo(Oil.getLiquid(1000)).add("soul_soil",750, 16);
        CENTRIFUGING.RB().ii(of(GTCoreTags.RUBBER_LOGS, 16)).io(DUST.get(Carbon, 8), new ItemStack(StickyResin, 8), new ItemStack(Plantball, 6)).fo(Methane.getGas(4000)).add("rubber_log",1562, 16);
        CENTRIFUGING.RB().fi(Hydrogen.getGas(4000)).fo(Deuterium.getGas(1000)).add("hydrogen",300, 20);
        CENTRIFUGING.RB().fi(Deuterium.getGas(4000)).fo(Tritium.getGas(1000)).add("deuterium",300, 20);
        CENTRIFUGING.RB().fi(Helium.getGas(16000)).fo(Helium3.getGas(1000)).add("helium",900, 20);
        add(Glowstone, 16,1562, 16);
        add(Redstone, 20, 1750);
        CENTRIFUGING.RB().ii(of(MAGMA_BLOCK, 64)).fo(Lava.getLiquid(64000)).add("magma_block",187, 16);
        CENTRIFUGING.RB().ii(of(ROTTEN_FLESH, 16)).io(new ItemStack(LEATHER, 4), new ItemStack(SLIME_BALL)).fo(Methane.getGas(4000)).add("rotten_flesh",375, 16);
        ...
        CENTRIFUGING.RB().ii(of(DUST.getMaterialTag(Endstone), 64)).io(new ItemStack(Items.SAND, 48), DUST.get(Tungsten, 1)).fo(Helium.getGas(4000), Helium3.getGas(4000)).add("endstone_dust",19500,16);
    ...
    private static void addMethaneRecipe(Item input, int inputAmount){
        CENTRIFUGING.RB().ii(of(input, inputAmount)).fo(Methane.getGas(1000)).add(AntimatterPlatformUtils.getIdFromItem(input).getPath() + "_into_methane",2250, 16);
    }

    private static void addMethaneRecipe(Item input, int inputAmount, ItemStack extra){
        CENTRIFUGING.RB().ii(of(input, inputAmount)).io(extra).fo(Methane.getGas(1000)).add(AntimatterPlatformUtils.getIdFromItem(input).getPath() + "_into_methane",2250, 16);
    }
